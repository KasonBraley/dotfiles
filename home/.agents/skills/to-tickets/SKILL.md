...
name: to-tickets
description: >-
  Break a plan, spec, or the current conversation into a set of tracer-bullet tickets, each declaring its
  blocking edges, published to the configured tracker — edges as text in one file per ticket locally, or
  native blocking links on a real tracker.
...
conventions established in context. Prepare the ticket files below before asking for missing publication details. Setup
is optional and does not block drafting.
...
Write the draft ticket files so the proposed scope, acceptance criteria, and blocking edges are reviewable. Mark them
`draft` while material decisions or requested approval are pending; set `ready-for-agent` only when publishing settled
tickets. If the user requested review before publishing, or a material scope choice remains, ask about that choice.
Reuse an approved breakdown; when the user has delegated slicing and authorized publication, proceed without another
approval round.

### 5. Publish the tickets to the configured tracker

Publish the tickets when authorized and material scope choices are settled. **How** depends on the configured or
user-supplied tracker — the tickets are the same either way, only the shape of the blocking edges changes:
...
- **A real issue tracker (GitHub, Linear, …)** → publish one issue per ticket in dependency order (blockers first) so
  ...
  `ready-for-agent` triage label unless instructed otherwise — the tickets are agent-grabbable by construction.

The **frontier** identifies tickets available for later implementation: those whose blockers are all done. This skill
finishes with ticket paths or published identifiers and blocking links; implement them only if also requested.

Leave parent issues unchanged unless the user explicitly requests a parent update.
